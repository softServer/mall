module.exports = {
    ...
    setPageNavbar: function() {
        var r = this, s = this.currentPage, e = getApp().core.getStorageSync("_navbar");
        e && n(e);
        var t = !1;
        for (var o in r.navbarPages) if (s.route == r.navbarPages[o]) {
            t = !0;
            break;
        }
        function n(e) {
            var t = !1;
            for (var o in e.navs) {
                var n = e.navs[o].url, a = s.route || s.__route__ || null;
                for (var i in n = e.navs[o].new_url, s.options) getApp().helper.inArray(i, [ "scene", "user_id", "uid" ]) || (-1 == a.indexOf("?") ? a += "?" : a += "&", 
                a += i + "=" + s.options[i]);
